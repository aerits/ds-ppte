...
fn main() {
    ...
    let mut game = BuyoBuyo::new(6, 12, Randomizer::new(4, 0), Tuning::new());
    ...
    while apt.main_loop() {
        ...
        if hid.keys_down().contains(KeyPad::A) {
            ...
            game.input_rotation_left();
        }
        if hid.keys_down().contains(KeyPad::B) {
            game.input_rotation_right();
        }
        ...
        unsafe {
            ...
            let buf  = C2D_TextBufNew(4096);
            C2D_TextParse(text.as_mut_ptr(), buf, game.score().to_string().as_ptr());
            ...
            for (v, s) in game.get_controlled_block() {
                let color = match s {
                    blockstackers_core::Sprite::Wall => C2D_Color32(0, 0, 0, 255),
                    blockstackers_core::Sprite::BuyoRed => C2D_Color32(255, 0, 0, 255),
                    blockstackers_core::Sprite::BuyoBlue => C2D_Color32(0, 0, 255, 255),
